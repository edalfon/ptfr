sim_usuraga <- function() {
  # simulate data
  tar_target(
    usuraga_data,
    sim_data_combinations(
      data_fn = sim_data_usuraga,
      n_draws = 100,
      sample_size = seq(20, 30, 2),
      base_rate = 0.05,
      effect_size = seq(0.05, 0.95, 0.15)
      #spillover_size = seq(0.1, 0.9, 0.2)
    )
  )

  # fit model to simulated data
  tar_target(usuraga_pvals, sim_fit_pvalues(usuraga_data, fit_usuraga_lejano))
  #tar_target(withpvals_cercano, sim_fit_pvalues(acapa_data, fit_acapa_cercano))

  tar_target(usuraga_power_plot, plot_power(usuraga_pvals))
...
sim_data_usuraga <- function(
  ...
  sim_data <- tibble::tibble(rowid = 1:sample_size) |>
    mutate(
      ...
      log_odds_tto = qlogis(0.33) + -0.1 * edad_s + 0.9 * amistad_s + 0.7 * educ_s,
...
x <- function() {
  devtools::load_all()
  sim_data <- sim_data_usuraga(round(runif(1) * 500))
...
fit_usuraga_cercano <- function(sim_data) {
...
fit_usuraga_lejano <- function(sim_data) {
