sim_siviru <- function() {
  ...
  tar_target(
    ...
    sim_data_combinations(
      data_fn = sim_data_siviru,
      n_draws = 100,
      sample_size = seq(30, 120, 10),
      base_rate = 0.529,
  ...
  tar_target(siviru_pvals, sim_fit_pvalues(siviru_data, fit_siviru_lejano))
  #tar_target(withpvals_cercano, sim_fit_pvalues(acapa_data, fit_acapa_cercano))

  tar_target(siviru_power_plot, plot_power(siviru_pvals))
...
x <- function() {
  devtools::load_all()
  sim_data <- sim_data_siviru(round(runif(1) * 500))
...
fit_siviru_cercano <- function(sim_data) {
...
fit_siviru_lejano <- function(sim_data) {
