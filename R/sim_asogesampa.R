sim_asogesampa <- function() {
  # simulate data
  tar_target(
    asogesampa_data,
    sim_data_combinations(
      data_fn = sim_data_asogesampa,
      n_draws = 100,
  ...
  tar_target(
    asogesampa_pvals,
    sim_fit_pvalues(asogesampa_data, fit_asogesampa_lejano)
  )
  #tar_target(withpvals_cercano, sim_fit_pvalues(acapa_data, fit_acapa_cercano))

  tar_target(asogesampa_power_plot, plot_power(asogesampa_pvals))
...
x <- function() {
  devtools::load_all()
  sim_data <- sim_data_asogesampa(round(runif(1) * 500))
...
fit_asogesampa_cercano <- function(sim_data) {
...
fit_asogesampa_lejano <- function(sim_data) {
