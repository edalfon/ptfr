sim_mariposas <- function() {
  # simulate data
  tar_target(
    mariposas_data,
    sim_data_combinations(
      data_fn = sim_data_mariposas,
      n_draws = 100,
  ...
  tar_target(mariposas_pvals, sim_fit_pvalues(mariposas_data, fit_mariposas_lejano))
  #tar_target(withpvals_cercano, sim_fit_pvalues(acapa_data, fit_acapa_cercano))

  tar_target(mariposas_power_plot, plot_power(mariposas_pvals))
...
sim_data_mariposas <- function(
  ...
  sim_data <- tibble::tibble(rowid = 1:sample_size) |>
    mutate(
      ...
      log_odds_tto = qlogis(0.33) + -0.1 * edad_s + 0.2 * amistad_s + 0.3 * educ_s,
...
x <- function() {
  devtools::load_all()
  sim_data <- sim_data_mariposas(round(runif(1) * 500))
...
fit_mariposas_cercano <- function(sim_data) {
...
fit_mariposas_lejano <- function(sim_data) {
