sim_raices <- function() {
  ...
  tar_target(
    ...
    sim_data_combinations(
      data_fn = sim_data_raices,
      n_draws = 100,
      sample_size = seq(30, 150, 10),
      base_rate = 0.529,
      effect_size = seq(0.05, 0.40, 0.05)
      #spillover_size = seq(0.1, 0.9, 0.2)
    )
  )

  tar_target(raices_pvals, sim_fit_pvalues(raices_data, fit_raices_lejano))

  tar_target(raices_power_plot, plot_power(raices_pvals))

  tar_target(
    raices_data_accidente,
    sim_data_combinations(
      data_fn = sim_data_raices,
      n_draws = 100,
      sample_size = seq(30, 140, 10),
      base_rate = 0.041,
      effect_size = seq(0.01, 0.1, 0.02)
      #spillover_size = seq(0.1, 0.9, 0.2)
    )
  )
  ...
  tar_target(
    raices_pvals_accidente,
    sim_fit_pvalues(raices_data_accidente, fit_raices_lejano)
  )
  #tar_target(withpvals_cercano, sim_fit_pvalues(acapa_data, fit_acapa_cercano))

  tar_target(raices_power_plot_accidente, plot_power(raices_pvals_accidente))
...
x <- function() {
  devtools::load_all()
  sim_data <- sim_data_raices(round(runif(1) * 500))
