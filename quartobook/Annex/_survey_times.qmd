```{r}
knitr::kable(
  data.frame(`Mean Time (minutes)` = mean(survey_times$tot_mins)),
  caption = "Mean Time (minutes)",
  digits = 1
)
```

```{r}
# survey_times |> efun::denstogram(tot_mins)

# Show summary quantiles in a nicely formatted table
knitr::kable(
  as.data.frame(
    t(quantile(survey_times$tot_mins, probs = c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1)))
  ),
  caption = "Summary of Times (minutes)",
  digits = 1
)
```

```{r, fig.width=7, fig.height=4, out.width='100%'}
survey_times |>
  ggplot(aes(x = tot_mins)) +
  geom_density(fill = "#3182bd", alpha = 0.6, color = "#08519c") +
  geom_rug() +
  labs(
    x = "Time (minutes)",
    y = "Density"
  ) +
  theme_minimal(base_size = 14)
```

```{r, fig.width=7, fig.height=4, out.width='100%'}
survey_times |>
  ggplot(aes(x = tot_mins, fill = beneficia)) +
  geom_density(alpha = 0.7, color = NA) +
  geom_rug() +
  labs(
    x = "Time (minutes)",
    y = "Density"
  ) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "top")
```

```{r}
# Cross-tabulate beneficia by created_by
survey_times |>
  count(beneficia, created_by) |>
  tidyr::pivot_wider(
    names_from = beneficia,
    values_from = n,
    values_fill = 0
  ) |>
  knitr::kable(
    caption = "Cross-tabulation of Created By by Beneficia"
  )
```

```{r, fig.width=7, fig.height=4, out.width='100%'}
survey_times |>
  ggplot(aes(x = tot_mins, fill = created_by)) +
  geom_density(alpha = 0.7, color = NA, size = 1) +
  geom_rug() +
  labs(
    x = "Time (minutes)",
    y = "Density"
  ) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "top") +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))
```


```{r, fig.width=7, fig.height=7, out.width='100%'}
survey_times |>
  ggplot(aes(x = tot_mins, fill = created_by)) +
  geom_density(alpha = 0.7, color = NA, size = 1) +
  geom_rug() +
  facet_grid(rows = vars(beneficia)) +
  labs(
    x = "Time (minutes)",
    y = "Density"
  ) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "top") +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))
```
